...
TUNING_LOSS_NAME = "mean_quadratic_ellipse_sincos_angle_norm512"
...
def tune_detector(
    ...
    *,
    ...
    Scoring uses geometry ``/512`` plus quadratic ``sin(2θ)/cos(2θ)`` angle residual.
    ...
    print(
        ...
        f"loss=sincos (sincos(2θ) embedding residual, geometry /512), "
    ...
    best_params["tuning_loss"] = TUNING_LOSS_NAME
...
def train_pipeline(
    ...
    *,
    ...
    Tuning loss is always sincos(2θ) with geometry normalized by 512.
    Hyperparameter search defaults to every ``tune_frame_stride``-th frame
    in ``[frame_min, frame_max]`` (e.g. 6, 11, …, 41).
    ``model_out`` defaults to ``splits/detector_model.json``.
    """
    ...
    print(
        ...
        f"(tune stride={tune_frame_stride}, tuning_loss=sincos)"
    )
    ...
    artifact_path = Path(model_out) if model_out is not None else split_path.parent / "detector_model.json"
...
def load_trained_detector(artifact_path: Path | str = "splits/detector_model.json") -> AblationEdgeDetector:
    """Load tuned detector params from ``detector_model.json``."""
